...
class AddPipeline(Protocol):
    def pipe(
        ...
        ticket: Ticket,
    ) -> AsyncPipeOut[AddPipelineOutput]: ...


class QwenImageEditAddPipeline:
    def __init__(
    ...
    def pipe(
        ...
        pipe = Pipe[AddPipelineOutput]()
        inp, out = PipeIn(pipe), AsyncPipeOut(pipe)
        ticket.use(partial(self._run,
                           character, asset, prompt, negative_prompt, inp))
        return out
    ...
    def _run(
        ...
        prompt: str,
        negative_prompt: str | None,
        output: PipeIn[AddPipelineOutput],
    ) -> None:
        try:
            result = self._qwen(
                image=[character.image, asset.image],
                prompt=prompt,
                negative_prompt=negative_prompt,
                on_step=_cancel_on_closed(output),
            )
            images = cast(list[Image.Image], result)
            output.pipe(AddPipelineOutput(character=Character(images[0])))
            output.close()
