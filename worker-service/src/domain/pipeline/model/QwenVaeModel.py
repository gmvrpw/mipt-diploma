...
class QwenVaeModel(Model):
    def __init__(self, device: Device, model_dir: str, hf_path: str) -> None:
        ...
        self._vae: AutoencoderKLQwenImage | None = None
    ...
    def load(self) -> None:
        ...
        self._latents_std = torch.tensor(latents_std).view(1, z_dim, 1, 1, 1)
    ...
    @torch.no_grad()
    def pipe_encode(self, image: torch.Tensor) -> torch.Tensor:
        self._device.load(self)
        assert self._vae is not None
        assert self._latents_mean is not None
        assert self._latents_std is not None

        image = image.to(device=DEVICE, dtype=DTYPE)
        (latent_dist,) = self._vae.encode(image, return_dict=False)
        ...
        out = (latents - latents_mean) / latents_std
